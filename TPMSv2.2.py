...
class RS232Reader:
    ...
    def read_serial_data(self):
        ...
        try:
            ...
            sys.stdout.write(f'Error with handshake request: {e}')
        ...
        try:
            ...
            sys.stdout.write(f'Error: Failed to read serial data. {str(e)}')
        finally:
            ser.close()
...
class DeviceData:
    ...
    def compile_pub_message(self, global_mystring):
        ...
        # new_character = ';'
        #mystring = ""
        # mystring = mystring[:index] + new_character + mystring[index + 1:]
...
#------------- #V2 --------------------------------------------------------------------------------------
    ...
    # mystring = ""
    # mystring = mystring[:index] + new_character + mystring[index + 1:]
    # pub_message = f"{dt.now().strftime('%Y-%m-%d %H:%M:%S')};{self.latitude};{self.longitude};{self.imei};{mystring}"
    # sys.stdout.write(pub_message)

    # mqtt_client.run(pub_message)
